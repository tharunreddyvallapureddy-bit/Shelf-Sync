...
export const OwnerPortal = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 animate-fadeIn">
      ...
      <div className="bg-slate-900 dark:bg-slate-900 bg-white rounded-3xl p-6 text-slate-900 dark:text-white shadow-2xl border border-slate-200 dark:border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <span className="px-2.5 py-0.5 rounded-full bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 font-extrabold text-[11px] border border-emerald-500/30 uppercase tracking-widest flex items-center gap-1">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div onClick={() => setStatusFilter('all')} className={`p-5 rounded-3xl border transition cursor-pointer ${statusFilter === 'all' ? 'bg-slate-900 text-white border-slate-800 shadow-xl' : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 hover:border-slate-400'}`}>
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Total Store SKUs</span>
            <Package className="w-5 h-5 text-slate-400" />
          </div>
          <div className="mt-3 flex items-baseline gap-2">
            <span className="text-3xl font-black text-slate-900 dark:text-white">{totalCount}</span>
            <span className="text-xs text-slate-500 dark:text-slate-400">Active Shop Catalog</span>
          </div>
        </div>

        <div onClick={() => setStatusFilter('available')} className={`p-5 rounded-3xl border transition cursor-pointer ${statusFilter === 'available' ? 'bg-emerald-950 text-emerald-200 border-emerald-800 shadow-xl' : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 hover:border-slate-400'}`}>
          <div className="flex items-center justify-between">
          ...
          <div className="mt-3 flex items-baseline gap-2">
            <span className="text-3xl font-black text-emerald-600 dark:text-emerald-400">{inStockCount}</span>
            <span className="text-xs text-slate-500 dark:text-emerald-300/80">Ready for Orders</span>
          </div>
        </div>

        <div onClick={() => setStatusFilter('low_stock')} className={`p-5 rounded-3xl border transition cursor-pointer ${statusFilter === 'low_stock' ? 'bg-amber-950 text-amber-200 border-amber-800 shadow-xl' : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 hover:border-slate-400'}`}>
          <div className="flex items-center justify-between">
          ...
          <div className="mt-3 flex items-baseline gap-2">
            <span className="text-3xl font-black text-amber-600 dark:text-amber-400">{lowStockCount}</span>
            <span className="text-xs text-slate-500 dark:text-amber-300/80">Below Safety Threshold</span>
          </div>
        </div>

        <div onClick={() => setStatusFilter('out_of_stock')} className={`p-5 rounded-3xl border transition cursor-pointer ${statusFilter === 'out_of_stock' ? 'bg-rose-950 text-rose-200 border-rose-800 shadow-xl' : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 hover:border-slate-400'}`}>
          <div className="flex items-center justify-between">
          ...
          <div className="mt-3 flex items-baseline gap-2">
            <span className="text-3xl font-black text-rose-600 dark:text-rose-400">{outOfStockCount}</span>
            <span className="text-xs text-slate-500 dark:text-rose-300/80">Immediate Restock</span>
          </div>
        </div>
      </div>

      {/* Demand Analytics Section */}
      <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 space-y-4 shadow-md">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs text-slate-500 dark:text-slate-400">Synced from Customer Web App</span>
        </div>

        {demandInsights.length === 0 ? (
          <div className="p-4 rounded-2xl bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-center text-xs text-slate-500 dark:text-slate-400">
            No active restock alert requests from customers yet.
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            {demandInsights.map(({ product, demandCount }) => (
              <div key={product.id} className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <img src={product.image} alt={product.name} className="w-10 h-10 rounded-xl object-cover" />
                  <div>
                    <span className="font-bold text-xs text-slate-900 dark:text-white block">{product.name}</span>
                    <span className="text-[11px] text-rose-600 dark:text-rose-400 font-semibold">Currently Out of Stock</span>
                  </div>
                </div>
                <div className="text-right">
                  <span className="text-sm font-black text-amber-600 dark:text-amber-400 block">{demandCount}</span>
                  <span className="text-[10px] text-slate-500 dark:text-slate-400">Waiting</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-2xl space-y-4 p-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h2 className="text-lg font-black text-slate-900 dark:text-white">Live Product Catalog</h2>
            <p className="text-xs text-slate-500 dark:text-slate-400">Inline quantity controls and restock date scheduler</p>
          </div>
          ...
          <div className="relative">
            ...
            <input
              ...
              className="pl-9 pr-4 py-2 rounded-xl bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-xs text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-emerald-500"
            />
          </div>
        </div>
                    ...
                    <tr key={p.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition">
                      <td className="py-4 px-6">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <span className="text-[11px] text-slate-500 dark:text-slate-400">{p.brand} • {p.category} ({p.unit})</span>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="py-4 px-6">
                        <div className="flex items-center justify-center gap-2">
                          <button
                            ...
                            className="w-8 h-8 rounded-xl bg-slate-200 dark:bg-slate-800 hover:bg-slate-300 dark:hover:bg-slate-700 font-extrabold text-slate-900 dark:text-white flex items-center justify-center transition"
                          >
                            -
                          </button>
                          ...
                          <input
                            ...
                            className={`w-16 text-center py-1.5 rounded-xl font-extrabold text-sm border focus:outline-none focus:ring-2 focus:ring-emerald-500 ${
                              isOut
                                ...
                                : isLow
                                ? 'border-amber-500 bg-amber-100 dark:bg-amber-950 text-amber-900 dark:text-amber-200'
                      ...
                      <td className="py-4 px-6">
                        <div className="flex items-center justify-between gap-2">
                          ...
                          <button
                            ...
                            className="p-1.5 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-800 text-emerald-600 dark:text-emerald-400 transition"
                          >
                      ...
                      <td className="py-4 px-4 text-right">
                        <button
                          ...
                          className="p-2 rounded-xl text-slate-400 hover:text-rose-600 hover:bg-rose-100 dark:hover:bg-rose-950 transition"
                        >
