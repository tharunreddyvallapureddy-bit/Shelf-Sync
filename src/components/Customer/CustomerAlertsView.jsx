...
import { Bell, Clock, CheckCircle2, AlertCircle, ShoppingBag } from 'lucide-react';
...
export const CustomerAlertsView = () => {
  ...
  // Filter subscriptions for current logged in user
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        <div>
          <h3 className="font-extrabold text-slate-900 dark:text-white text-base">My Restock Alerts</h3>
          <p className="text-xs text-slate-400">Products you asked to be notified about when restocked</p>
        </div>
      </div>

      {userSubs.length === 0 ? (
        <div className="text-center py-10 bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/80 dark:border-slate-800 space-y-2">
          <Bell className="w-8 h-8 text-slate-300 mx-auto" />
          <p className="font-bold text-slate-700 dark:text-slate-300 text-xs">No active restock alerts</p>
